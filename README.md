# SaveTag — Official Support & Privacy Site

Static support and privacy site for the iOS app **SaveTag: AI Social Bookmarks**
...
`index.html` and `privacy.html` are **generated** — edit `src/`, never the built
pages. Each locale writes its one-sentence summary once, in `foot`; the builder
...
`check.py` asserts:
1. all 50 shipped locales have complete support + privacy content, with the same
   number of chips, FAQ entries and policy sections as English,
